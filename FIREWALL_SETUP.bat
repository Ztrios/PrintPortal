...
echo  =========================================
echo  This will allow port 5000 on the local network.
echo  You must run this as Administrator.
echo.
pause

netsh advfirewall firewall add rule name="PrintPortal" dir=in action=allow protocol=TCP localport=5000
if %errorlevel% neq 0 (
